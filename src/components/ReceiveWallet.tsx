...
export default function ReceiveWallet({
  ...
  const handleReceiveWallet = async (walletName: string) => {
    const _provider = new ethers.BrowserProvider(
      walletName === 'MetaMask' ? window.ethereum : window.okxwallet,
    )
  ...
  return (
    <Dropdown className="bg-[#c2c2c2] min-w-[7.75rem]">
      <DropdownTrigger>
        <Button
          as="div"
          className="mt-[0.5rem] border border-black rounded-md w-[8rem] text-left pl-[0.5rem] h-[1.3rem]"
        >
      ...
      {!isConnected ? (
        ...
        >
          {walletList.map(wallet => (
            <DropdownItem key={wallet.walletName}>
              {wallet.walletName}
            </DropdownItem>
          ))}
        </DropdownMenu>
